export const Button = ({ className = "", props, children, text }) => {
  return (
    <button
      {...props}
    ...
    >
      {children}
      <div className="absolute left-1/2   -top-8 transform -translate-x-1/2 bg-customGrey-500 text-white text-[12px] p-1 px-2 border    border-customGrey-650 rounded-md opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300">
        {text}
      </div>
    </button>
  );
